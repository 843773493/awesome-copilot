...
适用于任何项目的 GitHub Copilot 代码审查综合指南。这些指南遵循提示工程的最佳实践，提供了一种结构化的方法来审查代码质量、安全性、测试和架构。
...
进行代码审查时，请按以下顺序优先处理问题：

### 🔴 严重问题（阻止合并）
- **安全性**：漏洞、暴露的密钥、认证/授权问题
- **正确性**：逻辑错误、数据损坏风险、竞争条件
- **重大变更**：在不进行版本控制的情况下更改 API 合约
- **数据丢失**：数据丢失或损坏的风险
...
1. **要具体**：引用具体的行号、文件名并提供具体示例
2. **提供上下文**：解释为何存在问题以及潜在影响
3. **建议解决方案**：在适用时展示修正后的代码，而不仅仅是指出问题
4. **建设性**：专注于改进代码，而不是批评作者
5. **认可良好实践**：认可良好编写的代码和聪明的解决方案
6. **务实**：并非每个建议都需要立即实施
7. **分组相关评论**：避免对同一主题的多次评论
...
def process_user(user_id):
    ...
    try:
        ...
        raise UserNotFoundError(f"用户 {user_id} 未在数据库中找到")
...
- **敏感数据**：代码或日志中无密码、API 密钥、令牌或个人身份信息（PII）
- **输入验证**：所有用户输入都经过验证和清理
...
- **更新 README**：在添加功能或更改设置时更新 README
- **重大变更**：清晰记录任何重大变更
- **示例**：为复杂功能提供使用示例
...
3. **业务逻辑规则**
   ...
   - 示例： "进行代码审查时，请检查数据处理前是否获得用户同意"

4. **团队规范**
   - 示例： "进行代码审查时，请验证提交信息是否遵循约定提交格式"
   - 示例： "进行代码审查时，请检查分支名称是否遵循模式：类型/工单描述"

## 额外资源

如需了解更有效的代码审查和 GitHub Copilot 定制信息：
...
进行代码审查时，请应用以下提示工程原则，来自 [GitHub Copilot 文档](https://docs.github.com/en/copilot/concepts/prompting/prompt-engineering)：

1. **先泛后专**：从高层架构审查开始，再深入实现细节
2. **提供示例**：在建议更改时引用代码库中的类似模式
3. **分解复杂任务**：将大型 PR 分为逻辑块进行审查（安全性 → 测试 → 逻辑 → 风格）
4. **避免歧义**：明确指出您正在处理的文件、行号和问题
5. **指示相关代码**：引用可能受更改影响的相关代码
6. **实验和迭代**：如果初始审查遗漏了某些内容，请再次使用聚焦问题进行审查
